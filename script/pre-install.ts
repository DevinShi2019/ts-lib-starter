...
const renameFiles: any[] = [
  // ["src/library.ts", "src/__libraryname__.ts"],
  // ["test/library.test.ts", "test/__myLibName__.test.ts"]
]
...
function modifyContents(libraryName: string, username: string, usermail: string) {
  ...
  try {
    const changes = replace.sync({
      files,
      from: [/__myLibName__/g, /__userName__/g, /__userEmail__/g],
...
function renameItems(libraryName: string) {
  ...
  renameFiles.forEach(function(files) {
    ...
    let newFilename = files[1].replace(/___myLibName_/g, libraryName)
    mv(
